...
public class _Doc implements Comparable<_Doc> {
	...
	int m_BitotalLength; //The total length of the document for Bigram.
	...
	//for Bi Gram
	private _SparseFeature[] m_x_bisparse; // sparse representation of features: default value will be zero.
	...
	//Constructor.
		public _Doc (int ID, String source, String author,String date){
			this.m_ID = ID;
			this.m_source = source;
			this.m_y_label = 0;
			this.m_totalLength = 0;
			this.m_BitotalLength = 0;
			m_author = author;
			m_date = date;
			m_topics = null;
			m_sstat = null;
	...
	//Get the total number of tokens in a document.
	public int getTotalBiDocLength(){
			return this.m_BitotalLength;
		}
	...
	//Create the sparse vector for the document.
		public void createBiSpVct(HashMap<Integer, Double> spVct) {
			m_x_bisparse = Utils.createSpVct(spVct);
			for(_SparseFeature fv:m_x_bisparse)
				m_BitotalLength += fv.getValue();
		}
